...
//
// Ce header contient TOUTE la logique du kernel (physique, combat, règles,
// reset, tick complet d'un match). Il compile :
//   - en CUDA (inclus par boxing_kernel.cu, fonctions __device__)
//   - en C++ pur (tools/cpu_check.cpp) -> testé tick par tick contre sim_ref
//     sur n'importe quelle machine, sans GPU (tests/test_cpu_kernel_check.py)
//
...
JD double clampd(double v, double lo, double hi) {
    return v < lo ? lo : (v > hi ? hi : v);
}

JD double wrap_degrees(double a) {
    a = fmod(a, 360.0);
    if (a >= 180.0) a -= 360.0;
    if (a < -180.0) a += 360.0;
    return a;
...
// ------------------------------------------------------------------ mouvement
JD double move_speed(int sprinting, int amp) {
...
JD void move_flying(P &p, double strafe, double forward, double friction) {
...
JD void move_entity(P &p, double dx, double dy, double dz,
                    double ax, double az) {
...
JD void living_update_movement(P &p, double strafe_in, double forward_in,
...
// --------------------------------------------------------------------- combat
JD double ray_aabb(double ox, double oy, double oz,
...
JD void knock_back(P &t, double rx, double rz) {
