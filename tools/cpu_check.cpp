...
// params.txt  : 32 floats (SimConfig.as_floats), un par ligne
...
int main(int argc, char **argv) {
    ...
    pr.post_sprint_hit_stop = (float)pv[5];
    pr.cps_min = (float)pv[6]; pr.cps_max = (float)pv[7];
    pr.rot_min = (float)pv[8]; pr.rot_max = (float)pv[9];
    pr.delay_min = (float)pv[10]; pr.delay_max = (float)pv[11];
    pr.jitter = (float)pv[12];
    pr.r_hit = (float)pv[13]; pr.r_hurt = (float)pv[14];
    pr.r_win = (float)pv[15]; pr.r_dist = (float)pv[16];
    pr.randomize = (float)pv[17];
    pr.spawn_gap = (float)pv[18];
    pr.kb_h = (float)pv[19];
    pr.kb_v = (float)pv[20];
    pr.kb_idle = (float)pv[21];
    pr.r_sprint_hit = (float)pv[22];
    pr.r_trade_penalty = (float)pv[23];
    pr.r_combo = (float)pv[24];
    pr.combo_window = (float)pv[25];
    pr.combo_cap = (float)pv[26];
    pr.smooth_min = (float)pv[27];
    pr.smooth_max = (float)pv[28];
    pr.r_combo_drop = (float)pv[29];
    pr.combo_drop_min = (float)pv[30];
    pr.r_combo_pressure = (float)pv[31];
